...
const RecipeModal: React.FC<RecipeModalProps> = ({
    ...
    const checkIfFavourite = async () => {
      try {
        const response = await axios.get(
          `/rest/favorites/check?userId=${userId}&recipeId=${recipeId}`
        );
  ...
  const handleFavouriteClick = async () => {
    try {
      ...
      if (isFavourite) {
        await axios.delete(
          `/rest/favorites/remove?userId=${userId}&recipeId=${recipeId}`
        );
      } else {
        await axios.post(
          `/rest/favorites/add?userId=${userId}&recipeId=${recipeId}`
        );
      }
